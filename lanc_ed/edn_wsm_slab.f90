...
contains
 ...
 !---------------------------------------------------------------------
  ...
  subroutine get_Akw()
    ...
    integer                                                 :: iorb,jorb
    integer                                                 :: ispin,jspin
    complex(8),dimension(:,:,:,:,:,:),allocatable           :: Sreal_,Sreal,Smats_,Smats
    complex(8),dimension(:,:,:),allocatable                 :: Akreal,Akmats
    complex(8),dimension(:,:,:,:,:,:,:),allocatable         :: Gkreal,Gkmats
    real(8)                                                 :: wr_(Lreal),wr(Lw),wm_(Lmats),wm(Lw)
  ...
  !
    ...
    wm_ = pi/beta*(2*arange(1,Lmats)-1)
    wm  = pi/beta*(2*arange(1,Lw)-1)
    !
    allocate(kpath(3,3))
    kpath(1,:)=[0,0,-1]*pi
    kpath(2,:)=[0,0,0]*pi
    ...
    allocate(Hkr(Nlso,Nlso,Lk));Hkr=zero
    allocate(Gkreal(Lk,Nlat,Nspin,Nspin,Norb,Norb,Lw));Gkreal=zero
    allocate(Gkmats(Lk,Nlat,Nspin,Nspin,Norb,Norb,Lw));Gkmats=zero
    allocate(Akreal(Lk,Nlat,Lw));Akreal=zero
    allocate(Akmats(Lk,Nlat,Lw));Akmats=zero
    ...
    allocate(Smats_(Nlat,Nspin,Nspin,Norb,Norb,Lmats));Smats_=zero
    allocate(Sreal(Nlat,Nspin,Nspin,Norb,Norb,Lw));Sreal=zero
    allocate(Smats(Nlat,Nspin,Nspin,Norb,Norb,Lw));Smats=zero
    call read_sigma_real(Sreal_)
    call read_sigma_matsubara(Smats_)
    !
    do ilat=1,Nlat
      do ispin=1,Nspin
        do iorb=1,Norb
          call cubic_spline(wr_,Sreal_(ilat,ispin,ispin,iorb,iorb,:),wr,Sreal(ilat,ispin,ispin,iorb,iorb,:))
          call cubic_spline(wm_,Smats_(ilat,ispin,ispin,iorb,iorb,:),wm,Smats(ilat,ispin,ispin,iorb,iorb,:))
        enddo
      enddo
    enddo
    !
    ...
    do ik=1,Lk
        ...
        call dmft_gk_matsubara(Hkr(:,:,ik),1d0,Gkmats(ik,:,:,:,:,:,:),Smats)
        call eta(ik,Lk)
    enddo
    call stop_timer
    !
    do ispin=1,Nspin
      do iorb=1,Norb
        ...
        Akmats = Akmats - dimag(Gkmats(:,:,ispin,ispin,iorb,iorb,:))/pi/Nspin/Norb
      enddo
    enddo
    do ilat=1,Nlat
      call splot3d("Akw_real_nso"//reg(txtfy(ilat))//".dat",kgrid(:,3),wr,Akreal(:,ilat,:)) 
      call splot3d("Akw_matsubara_nso"//reg(txtfy(ilat))//".dat",kgrid(:,3),wm,Akmats(:,ilat,:)) 
    enddo
